...
if EMBEDDINGS_MODEL_SETTING == "OPENAI":
    ...
    embeddings = HuggingFaceEmbeddings(
        model_name=local_model_target,  # <-- Pass the dynamic variable here cleanly!
...
# Connect safely to the environment-targeted index partition
...
def query_telemetry_db(sql_query: str) -> str:
    ...
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(text(sql_query)).fetchmany(10)
